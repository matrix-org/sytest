...
if [ -d "/sytest" ]; then
    ...
    if [ -n "$BUILDKITE_BRANCH" ]; then
        ...
        # Otherwise, try and find out what the branch that the Synapse/Dendrite
        # checkout is using. Fall back to develop if it's not a branch.
        branch_name="$(git --git-dir=/src/.git symbolic-ref HEAD 2>/dev/null)" || branch_name="develop"

        if [ "$SYTEST_TARGET" == "dendrite" ] && [ branch_name == "master" ]; then
            # Dendrite uses master as its main branch. If the branch is master,
            # we probably want sytest develop
            branch_name="develop"
        fi
    fi
