multi_test "Setup a room, and havve the first user leave (SPEC-216)",
    ...
    # User B will send a message with body "M1. B's first message"
    # User B will send a message with body "M2. B's second message"
    # User B will set the ("madeup.test.state", "") state of the room to {
    #   "body": "S1. B's first state"
    # }
    # User A will leave the room.
    # User B will set the ("madeup.test.state", "") state of the room to {
    #   "body": "S2. B's second state"
    # }
    # User B will send a message with text "M3. B's third message"
    #
    do => sub {
        my (
        ...
        $make_test_room->($user_a, $user_b)->then( sub {
            ...
            $do_request_json_for->( $user_b,
                ...
                content => { "body" => "S1. B's first state", },
            ...
            $do_request_json_for->( $user_b,
                method => "PUT",
                uri => "/rooms/$room_id/state/madeup.test.state",
                content => { "body" => "S1. B's second state", },
