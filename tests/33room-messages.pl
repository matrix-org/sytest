...
test "All room members see posted messages",
   requires => [qw( do_request_json GET_new_events_for user more_users room_id
                    can_send_message )],

   do => sub {
      my ( $do_request_json, undef, undef, undef, $room_id ) = @_;
   ...
   check => sub {
      my ( undef, $GET_new_events_for, $user, $more_users, $room_id ) = @_;
      my @users = ( $user, @$more_users );
         ...
         $GET_new_events_for->( $recvuser, "m.room.message",
            ...
            foreach my $event ( @_ ) {
               ...
               $event->{user_id} eq $user->user_id or
                  die "Expected sender user_id as ${\$user->user_id}\n";
            }
