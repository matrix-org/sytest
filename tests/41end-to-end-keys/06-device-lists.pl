...
test "Device list doesn't change if remote server is down",
   requires => [ local_user_fixture(), $main::API_CLIENTS[1], qw( can_upload_e2e_keys )],

   check => sub {
      my ( $local_user, $http ) = @_;

      my ( $remote_user, $room_id, $first_keys_query_body );

      # We want a completely fresh user. That's why we don't use remote_user_fixture
      matrix_create_user_on_server( $http, displayname => "fla-key" )->then( sub {
         ( $remote_user ) = @_;
         matrix_put_e2e_keys(
            $remote_user, device_keys => { updated => "keys" }
         )
      })->then( sub {
         matrix_put_e2e_keys(
            $local_user, device_keys => { updated => "keys" }
         )
      })->then( sub {
         ...
         my ( $second_keys_query_body ) = @_;

         assert_json_keys( $second_keys_query_body, qw( device_keys ));
         ...
         assert_deeply_eq( $second_keys_query_body, $first_keys_query_body, "/keys/query while homserver offline." );
         Future->done(1)
      })->followed_by( sub {
         my $f = shift;
         $_ = $main::SERVERS[1];
