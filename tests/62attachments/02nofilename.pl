test "Can upload without a file name",
    ...
    do => sub {
        my ($http, $user) = @_;
        $http->do_request(
            method => "POST", full_uri => "/_matrix/media/v1/upload",
            content_type => "text/plain", content => "Test media file",
            params => {
...
test "Can download without a file name",
    ...
    check => sub {
        ...
        $http->do_request(
            ...
            not defined $disposition
                or die "Unexpected Content-Disposition header";
            Future->done(1);
        });
    };
